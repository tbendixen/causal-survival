...
```{r subgroup-cox-sw-plot, dependson="subgroup-cox-sw", fig.width=8, fig.asp=0.618}
...
```{r subgroup-cox-swV-plot, dependson="subgroup-cox-swv", fig.width=8, fig.asp=0.618}
ggplot(subgroup_iptw_surv, aes(x=time, y=surv)) + 
  geom_line(aes(y = surv00, colour = "1")) + 
  geom_line(aes(y = surv01, colour = "2")) + 
  geom_line(aes(y = surv10, colour = "3")) + 
...
doubly_cox <- function(formula, data, indices) {
  ...
  # fit of doubly robust Cox model
  # includes both weights and full vector of covariates
  mod <- coxph(Surv(survtime, event) ~ qsmk + sex + race + age + I(age * age) + as.factor(education)
                              + smokeintensity + I(smokeintensity * smokeintensity)
                              + smokeyrs + I(smokeyrs * smokeyrs) + as.factor(exercise)
                              + as.factor(active) + wt71 + I(wt71 * wt71), 
               weights = d$sw.a, data = d, model = TRUE)
  
  # predict and marginalize
  doubly_fit_qsmk0 <- avg_predictions(mod, newdata = transform(data, qsmk=0), 
                                    variables = list(survtime = sort(unique(data$survtime))), 
                                    vcov = FALSE, # don't compute SEs with delta method
                                    type = "survival")
  
  # predict and marginalize
  doubly_fit_qsmk1 <- avg_predictions(mod, newdata = transform(data, qsmk=1), 
                                    variables = list(survtime = sort(unique(data$survtime))), 
                                    vcov = FALSE, # don't compute SEs with delta method
                                    type = "survival")

  # collect
  doubly_fit_qsmk0$surv0 <- doubly_fit_qsmk0$estimate
  doubly_fit_qsmk1$surv1 <- doubly_fit_qsmk1$estimate

  return(c(doubly_fit_qsmk0$surv0, doubly_fit_qsmk1$surv1))
}

# doubly robust Cox bootstrap
(bootstrapped_results <- boot(data=nhefs,
                              statistic=doubly_cox,
                              R=n_R,
                              formula = qsmk ~ sex + race + age + I(age * age) + as.factor(education)
                              + smokeintensity + I(smokeintensity * smokeintensity)
                              + smokeyrs + I(smokeyrs * smokeyrs) + as.factor(exercise)
...
```{r dr-iptw-cox-plot, dependson="iptw-cox", fig.width=8, fig.asp=0.618}
# survival curves
(survival_plot <- ggplot(boot_samples, aes(x=time, y=surv)) + 
  stat_lineribbon(aes(y = surv0, colour = "0"), fill = "grey", alpha = 0.5, .width=.95) +
  stat_lineribbon(aes(y = surv1, colour = "1"), fill = "grey", alpha = 0.5, .width=.95) +
  ...
  scale_y_continuous(limits=c(0.6, 1), breaks=seq(0.6, 1, 0.2)) +
  ylab("Survival") + 
  ggtitle("Cox model (Doubly robust with bootstrap)") + 
  labs(colour="A:") +
  theme_classic() + 
  theme(legend.position="bottom"))
...
# Bonus: Visualizing IPTW "pseudo-population"
...
```{r pseudo-pop, fig.width=8, fig.asp=0.618}
library(MetBrewer)
library(patchwork)
...
## Pre- and post-weighting Table 1
...
### Unweighted
```{r pre-tab1, results='as-is'}
# not weighted
bal.tab(f, data = d)
```

### Weighted
```{r post-tab1, results='as-is'}
# weighted
bal.tab(f, data = d, weights = d$sw.a, s.d.denom=)
```

# Schoenfeld residuals

```{r sch-res}
### Fit doubly robust Cox, as above
nhefs$event <- nhefs$death
nhefs$id <- nhefs$seqn
...
```{r rmst}
# fit weighted Kaplan-Meier; we re-use weights from earlier
npfit <- survfit(Surv(survtime, event) ~ qsmk, weights = d$sw.a, data=d)

# restricted mean survival time with standard errors from time 0 to 120
print(npfit, print.rmean = TRUE, rmean = 120)

# compute confidence intervals "manually", assuming gaussian distribution
fit.table <- summary(npfit, print.rmean = TRUE, rmean = 120)$table |> 
  as.data.frame()
rmean <- fit.table["qsmk=0", "rmean"]                            # Estimated restricted mean for qsmk=0
rmean.se <- fit.table[["se(rmean)"]]                             # Estimated standard error
rmean + c(-1,1) * qnorm(.05/1.96, lower.tail=FALSE) * rmean.se   # 95% CI
